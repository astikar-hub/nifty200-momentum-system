...
def get_regime():
    ...
    regime = (price > dma200)
    monthly_regime = regime.resample("ME").last()  # Monthly end regime
...
def run_strategy(monthly_prices, regime, lookback, top_n):
    ...
    for i in range(lookback, len(monthly_prices)):
        ...
        # Calculate momentum
        momentum = (
            monthly_prices.iloc[i-1] / monthly_prices.iloc[i-lookback] - 1
        )
    ...
    # Shift weights to apply previous weights at the current time
...
def main():
    ...
    regime = get_regime()
    monthly_prices = prices.resample("ME").last()  # Resample to monthly data
    ...
    for lookback in LOOKBACK_LIST:
        for top_n in TOPN_LIST:
            equity, buy_signals, sell_signals = run_strategy(
                monthly_prices, regime, lookback, top_n
            )
